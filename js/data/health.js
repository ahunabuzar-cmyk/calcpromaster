...
const HEALTH_TOOLS = [
  { id: 'bmi', name: 'BMI Calculator', desc: 'Calculate Body Mass Index with category', kw: 'bmi calculator, body mass index, weight calculator',
  ...
  { id: 'bmr', name: 'BMR Calculator', desc: 'Basal metabolic rate — 3 formulas: Mifflin-St Jeor, Harris-Benedict, Katch-McArdle', kw: 'bmr calculator, basal metabolic rate, metabolism, mifflin, harris benedict, katch mcardle',
    inputs: [
  ...
  { id: 'calorie', name: 'Calorie Calculator', desc: 'Daily calorie needs (TDEE)', kw: 'calorie calculator, daily calories, tdee',
    inputs: [
  ...
  { id: 'body-fat', name: 'Body Fat Calculator', desc: 'Estimate body fat percentage (Navy method)', kw: 'body fat calculator, body fat percentage, fitness',
  ...
  { id: 'water-intake-health', name: 'Water Intake Calculator', desc: 'Daily water requirement', kw: 'water intake, hydration calculator, daily water',
  ...
  { id: 'ideal-body-weight', name: 'Ideal Weight Calculator', desc: 'Calculate ideal body weight (Devine)', kw: 'ideal weight, healthy weight, body weight',
  ...
  { id: 'heart-rate', name: 'Heart Rate Zone Calculator', desc: 'Target heart rate zones for exercise', kw: 'heart rate, target heart rate, cardio zones',
  ...
  { id: 'pregnancy', name: 'Pregnancy Due Date', desc: 'Calculate pregnancy due date', kw: 'pregnancy due date, pregnancy calculator, expected delivery',
  ...
  { id: 'ovulation', name: 'Ovulation Calculator', desc: 'Calculate ovulation date', kw: 'ovulation calculator, fertility, conception',
  ...
  { id: 'macros', name: 'Macronutrient Calculator', desc: 'Calculate daily macros (protein/carbs/fat)', kw: 'macro calculator, macronutrients, protein carbs fat',
  ...
  { id: 'lean-body-mass', name: 'Lean Body Mass', desc: 'Calculate lean body mass', kw: 'lean body mass, lbm, muscle mass',
  ...
  { id: 'bsa', name: 'Body Surface Area', desc: 'Calculate body surface area (Mosteller)', kw: 'body surface area, bsa, medical calculator',
  ...
  { id: 'waist-hip', name: 'Waist-to-Hip Ratio', desc: 'Calculate WHR and health risk', kw: 'waist to hip ratio, whr, body shape',
  ...
  { id: 'vo2-max', name: 'VO2 Max Calculator', desc: 'Estimate cardiovascular fitness', kw: 'vo2 max, cardiovascular fitness, aerobic capacity',
  ...
  { id: 'pregnancy-weight', name: 'Pregnancy Weight Gain', desc: 'Recommended pregnancy weight gain', kw: 'pregnancy weight gain, pregnancy weight, prenatal',
  ...
  { id: 'blood-pressure', name: 'Blood Pressure Category', desc: 'Check blood pressure category', kw: 'blood pressure, hypertension, bp category',
  ...
  { id: 'calorie-burn', name: 'Calorie Burn Calculator', desc: 'Calories burned by activity', kw: 'calorie burn, exercise calories, activity calculator',
  ...
  { id: 'sleep', name: 'Sleep Calculator', desc: 'Calculate optimal sleep times', kw: 'sleep calculator, sleep cycle, bedtime',
  ...
  { id: 'gfr', name: 'GFR Calculator', desc: 'Estimate kidney function', kw: 'gfr calculator, kidney function, egfr',
  ...
  { id: 'target-heart-rate', name: 'Target Heart Rate Zones', desc: 'Calculate your heart rate training zones', kw: 'target heart rate, heart rate zones, karvonen, training zones',
...
{ id: "pregnancy-due-date", name: "Pregnancy Due Date Calculator", desc: "Calculate estimated due date and current pregnancy week", kw: "pregnancy calculator, due date, conception date, pregnancy week", inputs: [{id:"lmp",label:"First Day of LMP (DD-MM-YYYY)",type:"text",def:"15-01-2026"},{id:"cycleLength",label:"Cycle Length (days)",type:"number",def:28}], calc: function(v) { var parts=v.lmp.split("-"); var d=new Date(parseInt(parts[2]),parseInt(parts[1])-1,parseInt(parts[0])); var due=new Date(d); due.setDate(due.getDate()+280+(v.cycleLength-28)); var today=new Date(); var dp=Math.max(0,Math.floor((today-d)/86400000)); var w=Math.floor(dp/7); var dy=dp%7; var rem=Math.max(0,Math.floor((due-today)/86400000)); return { result: "Due Date: "+due.toLocaleDateString(), chart: Charts.gauge(Math.min(w,40)*2.5,100), extra: "Week "+w+", Day "+dy+" | "+rem+" days remaining" }; }, steps: function(v) { var p=v.lmp.split("-"); var d=new Date(parseInt(p[2]),parseInt(p[1])-1,parseInt(p[0])); var due=new Date(d); due.setDate(due.getDate()+280+(v.cycleLength-28)); return ["Step 1: LMP = "+v.lmp,"Step 2: Due date = "+due.toLocaleDateString()]; } },
